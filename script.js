// Game State
        ...
        document.getElementById('startGame').addEventListener('click', () => {
            ...
            // Determine character (default to 'thai')
            const selectedCharacter = document.querySelector('.character-card.selected');
            gameState.character = selectedCharacter ? selectedCharacter.dataset.character : 'thai';
            
            // Set board avatar from selected card image if available, else local fallback, else SVG
            const holder = document.getElementById('character');
            (function(){
                let imgSrc = '';
                if (selectedCharacter) {
                    const imgEl = selectedCharacter.querySelector('.character-icon');
                    if (imgEl && imgEl.getAttribute('src')) imgSrc = imgEl.getAttribute('src');
                }
                if (!imgSrc) {
                    imgSrc = (gameState.character === 'buddha') ? './buddha.png' : './villager.png';
                }
                if (imgSrc) {
                    holder.innerHTML = '<img src="' + imgSrc + '" alt="avatar" />';
                } else {
                    // SVG fallback (legacy)
                    if (gameState.character === 'buddha') {
                        holder.innerHTML = `<svg viewBox="0 0 60 60" style="width: 100%; height: 100%;">
            ...
            // Hide start screen & init (ensure executed)
            document.getElementById('startScreen').classList.add('hidden');
            initGame();
